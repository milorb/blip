...
class Renderer {
 public:
  ...
  void render_rectangle(const Vec2& pos, const Vec2& dims, SDL_Color& color,
                        bool filled);
  ...
  void render_command_prompt_bar(std::string status, std::string& command,
                                 std::string user_input, Vec2& cursor_pos);
 ...
 private:
  ...
  void render_texture(SDL_Texture* texture,
                      const SDL_Rect& rect);  // file editor functions
